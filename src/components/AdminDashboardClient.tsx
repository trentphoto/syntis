...
export default function AdminDashboardClient() {
  ...
  useEffect(() => {
    // Simulate loading for client-side data fetches. Replace with real fetch logic.
    const t = setTimeout(() => setLoading(false), 700)
    return () => clearTimeout(t)
  }, [])

  const SkeletonBox = ({ className = "h-6 w-full rounded-md bg-muted/20 animate-pulse" }: { className?: string }) => (
          ...
          <CardContent>
            {loading ? (
              <div className="h-[300px] flex items-center justify-center">
                <SkeletonBox className="h-48 w-full max-w-4xl rounded-md bg-muted/20 animate-pulse" />
              </div>
            ) : (
              <ChartContainer
                config={{
              ...
              <TableBody>
                {loading
                  ? [1, 2, 3, 4, 5].map((i) => (
                      <TableRow key={i}>
                        <TableCell>
                          <SkeletonBox className="h-4 w-40 rounded-md bg-muted/20 animate-pulse" />
                        </TableCell>
                        <TableCell>
                          <SkeletonBox className="h-4 w-24 rounded-md bg-muted/20 animate-pulse" />
                        </TableCell>
                        <TableCell>
                          <SkeletonBox className="h-4 w-56 rounded-md bg-muted/20 animate-pulse" />
                        </TableCell>
                      </TableRow>
                    ))
                  : clientsData
            ...
            <CardContent>
              {loading ? (
                <div className="h-[200px] flex items-center">
                  <SkeletonBox className="h-40 w-full rounded-md bg-muted/20 animate-pulse" />
                </div>
              ) : (
                <ChartContainer
                  config={{
            ...
            <CardContent>
              {loading ? (
                <div className="h-[200px] flex items-center justify-center">
                  <SkeletonBox className="h-40 w-40 rounded-full bg-muted/20 animate-pulse" />
                </div>
              ) : (
                <ChartContainer
                  config={{
                ...
                {loading
                  ? [1, 2, 3].map((i) => (
                      <TableRow key={i}>
                        ...
                        <TableCell>
                          <SkeletonBox className="h-4 w-48 rounded-md bg-muted/20 animate-pulse" />
                        </TableCell>
                        <TableCell>
                          <SkeletonBox className="h-4 w-48 rounded-md bg-muted/20 animate-pulse" />
                        </TableCell>
                        <TableCell>
                          <SkeletonBox className="h-4 w-20 rounded-md bg-muted/20 animate-pulse" />
                        </TableCell>
                      </TableRow>
                    ))
                  : auditLogs.map((log) => (
